...
def nearest_neighbor_edge_centrality(GRAPH):
    edges = GRAPH.edges

    for edge in edges:
        node1_dc = GRAPH.degree(edge[0])
        node2_dc = GRAPH.degree(edge[1])

        nearest_neighbor_edge_centrality = (node1_dc + node2_dc - 2) / (abs(node1_dc - node2_dc) + 1) 
        print('Edge', edge,'with nearest neighbor edge_centrality ->', nearest_neighbor_edge_centrality)
...
if __name__ == '__main__':
    ...
    nearest_neighbor_edge_centrality(GRAPH)
