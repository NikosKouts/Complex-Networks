...
def Girvan_Newman(root, Graph, graph_edges):
    # Preorder Print
    print(Graph.nodes)
    # print(graph_edges)
    ...
    while nx.is_connected(Graph):
        ...
        delete_edge = graph_edges.popitem()
        node1 = delete_edge[0][0]
        node2 = delete_edge[0][1]
    ...
    left_edge_dic = graph_edges.copy()
    right_edge_dic = graph_edges.copy()
   
    # left child
    root.left = Node(left_subgraph)
    Girvan_Newman(root.left, left_subgraph, left_edge_dic)

    # right child
    root.right = Node(right_subgraph)
    Girvan_Newman(root.right, right_subgraph, right_edge_dic)
...
def main():
    ...
    graph_edges = nx.edge_betweenness_centrality(GRAPH)
    graph_edges = dict(sorted(graph_edges.items(), key=lambda item: item[1]))

    Girvan_Newman(root, GRAPH, graph_edges)
